...
import { RiRepeatLine, RiCheckLine } from "@remixicon/react"
  ...
  const defaultContent = (
    <>
      ...
      {!occurrence.allDay && segment.isStart && (
        <span className="hidden truncate text-muted-foreground @[8rem]:inline">
    ...
    <>
      {segment.isStart && (
        ...
        >
          <span
            aria-hidden
            className="h-2.5 w-0.5 rounded-full bg-foreground/40"
          />
        </span>
      )}
      {segment.isEnd && (
