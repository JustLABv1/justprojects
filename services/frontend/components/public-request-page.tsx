...
import { RiArrowLeftLine, RiCheckLine, RiLockLine } from "@remixicon/react"
...
import {
  ProjectRequestForm,
  type ProjectRequestFormResult,
} from "@/components/project-request-form"
...
export function PublicRequestPage({ requestSlug }: { requestSlug: string }) {
  ...
  const [error, setError] = useState<string>(() =>
    isApiConfigured ? "" : t("auth.apiRequired")
  )
      ...
      <div className="mx-auto max-w-3xl px-4 py-6 sm:px-6 sm:py-10">
        ...
        {loading ? (
          <div
            ...
            role="status"
          >
            {t("public.requestWorkspaceLoading")}
          </div>
        ) : error ? (
          <div className="mx-auto mt-20 max-w-md">
            <FeedbackNotice
              kind="error"
              title={t("public.requestWorkspaceNotAvailable")}
              message={error}
            />
          </div>
        ) : workspace ? (
          <section className="mt-14">
            ...
            <div className="mt-8 max-w-2xl">
              ...
              <h1 className="mt-5 text-4xl font-semibold tracking-tight sm:text-5xl">
                {t("public.requestWorkspaceTitle")}
              </h1>
              <p className="mt-4 text-base leading-relaxed text-muted-foreground">
                {t("public.requestWorkspaceDescription")}
              </p>
            </div>
                ...
                <div className="space-y-5">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h2 className="text-lg font-semibold">
                        {t("public.requestWorkspaceSuccessTitle")}
                      </h2>
                      <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
                        {t("public.requestWorkspaceSuccessDescription")}
                      </p>
                    </div>
                  </div>
                  <div className="rounded-2xl border bg-muted/30 p-4 text-sm text-muted-foreground">
                    ...
                    {submitted.requestToken && (
                      <code className="mt-3 block rounded-lg bg-background px-3 py-2 text-xs break-all text-foreground">
                        {submitted.requestToken}
                      </code>
                    )}
                  </div>
                  ...
                  >
                    {t("public.requestWorkspaceSubmitAnother")}
                  </Button>
                </div>
              ) : (
                <ProjectRequestForm
                  ...
                  onSubmitRequest={(input) =>
                    createWorkspaceProjectRequest(workspace.requestSlug, input)
                  }
